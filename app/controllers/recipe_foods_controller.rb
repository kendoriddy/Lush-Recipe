class RecipeFoodsController < ApplicationController
  ...
  def create
    ...
    if recipe_food.save
      redirect_to recipe_path(@recipe), notice: 'New ingredient added successfully.'
    else
  ...
  def destroy
    ...
    unless recipe_food.recipe.user == current_user
      flash[:alert] =
        'You can not delete the ingredient that you did not add'
      return redirect_to recipes_path
    end
    ...
    if recipe_food.destroy
      ...
      flash[:alert] = 'Ingredient deleting failed. Please try again.'
    end
